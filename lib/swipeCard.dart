...
class swipeCards extends StatefulWidget {
	swipeCards(this.data);
	List data;
	@override
	swipeCardsState createState() => new swipeCardsState(this.data);
}


class swipeCardsState extends State<swipeCards> with TickerProviderStateMixin {

	swipeCardsState(this.data);
	...
	Widget build(BuildContext context) {
		...
		List showndata = limiteddata.map( (item) {
	    ...
	    return new Container(
			...
			: // if there's no discovery items
			Text("Nothing Left", style: TextStyle(color: Colors.black, fontSize: 50.0)),
	    );
	}
...
SlideTransition swipeCardTransition (
	item,
    double bottom,
    double cardMargin,
